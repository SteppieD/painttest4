-- Migration to add missing tables for Supabase compatibility (FIXED VERSION)
...
BEGIN
    ...
    -- Check if companies table exists and has id column
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'companies') THEN
        RAISE NOTICE 'Companies table exists';
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'id') THEN
            RAISE NOTICE 'Companies.id column exists';
        ELSE
            RAISE NOTICE 'WARNING: Companies.id column does not exist!';
        END IF;
    ELSE
        RAISE NOTICE 'WARNING: Companies table does not exist!';
    END IF;
END $$;
...
-- =====================================================
-- COMPANY SUBSCRIPTIONS TABLE (Modified to handle missing foreign key)
-- =====================================================

-- First check if companies table exists before creating foreign key
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'companies') THEN
    ...
    ELSE
        -- Create without foreign key if companies table doesn't exist
        CREATE TABLE IF NOT EXISTS public.company_subscriptions (
          id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
          company_id INTEGER NOT NULL,
          plan_id TEXT NOT NULL REFERENCES subscription_plans(id),
        ...
        RAISE NOTICE 'Created company_subscriptions without foreign key to companies table';
    END IF;
END $$;
...
-- =====================================================
-- QUOTE USAGE TABLE (Modified to handle missing foreign key)
-- =====================================================

-- Similar approach for quote_usage
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'companies') THEN
        CREATE TABLE IF NOT EXISTS public.quote_usage (
          ...
          created_at TIMESTAMPTZ DEFAULT NOW(),
          CONSTRAINT fk_quote_usage_company
            FOREIGN KEY (company_id) 
            REFERENCES companies(id) 
            ON DELETE CASCADE
        );
    ELSE
        ...
        RAISE NOTICE 'Created quote_usage without foreign key to companies table';
    END IF;
END $$;
...
-- =====================================================
-- PROJECTS TABLE (Modified to handle missing foreign key)
-- =====================================================
...
BEGIN
    ...
    ELSE
        ...
        RAISE NOTICE 'Created projects without foreign key to companies table';
    END IF;
END $$;
...
-- =====================================================
-- COMPANY BRANDING TABLE (Modified to handle missing foreign key)
-- =====================================================
...
BEGIN
    ...
    ELSE
        ...
        RAISE NOTICE 'Created company_branding without foreign key to companies table';
    END IF;
END $$;
...
-- Function to get paint products for a company (modified to handle missing companies table)
...
BEGIN
  ...
  ELSE
    -- If no companies table, return empty result
    RETURN;
  END IF;
...
BEGIN
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'quote_usage') THEN
...
SELECT 
  ...
  (SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'companies')) as companies_table_exists;